...
new g_itemindex1
new g_itemindex2
new g_has_fast_energy[MAX_PLAYERS + 1] = {false, ...}
...
// Client general info
new Float:g_nn_energy[MAX_PLAYERS + 1]
new Float:cl_nn_controlling[33]
new cl_nn_weapon[MAX_PLAYERS + 1]
new bool:cl_is_bot[MAX_PLAYERS + 1] = {false, ...}
new bool:cl_nn_lowres[MAX_PLAYERS + 1] = {false, ...}
new NanoStatus:cl_nn_has[MAX_PLAYERS + 1] = {NANO_NO, ...}
new NanoStatus:cl_nn_had[MAX_PLAYERS + 1] = {NANO_NO, ...}
new bool:cl_added_velocity[MAX_PLAYERS + 1] = {false, ...}
new bool:cl_removed_shadow[MAX_PLAYERS + 1] = {false, ...}
new bool:cl_nn_zombie[MAX_PLAYERS + 1] = {false, ...}

// Nanosuit special info
new NanoSpdMode:cl_nn_sp_status[MAX_PLAYERS + 1]
new NanoSpeed:cl_nn_speed[MAX_PLAYERS + 1]
new NanoSpeedScreen:cl_nn_scr_speed[MAX_PLAYERS + 1]
new NanoModes:cl_nn_mode[MAX_PLAYERS + 1] = {NANO_ARMOR, ...}
new Float:cl_nn_energy[MAX_PLAYERS + 1]
new bool:cl_nn_critical[MAX_PLAYERS + 1]
new bool:cl_nn_online[MAX_PLAYERS + 1]
new cl_nn_counter[MAX_PLAYERS + 1] = {NANO_LOW_RES, ...}
new cl_nn_block_recharge[MAX_PLAYERS + 1]
new KnifeState:cl_nn_st_knife[MAX_PLAYERS + 1] = {KNIFE_NOT, ...}
new bool:cl_nn_st_jump[MAX_PLAYERS + 1] = {false, ...}
new cl_is_thrown[MAX_PLAYERS + 1] = {0, ...}
new Float:cl_nn_punch[MAX_PLAYERS + 1][3]
new bool:cl_nn_actual_shot[MAX_PLAYERS + 1] = {false, ...}
new cl_nn_shotgun_ammo[MAX_PLAYERS + 1]
new Float:cl_nn_damage_time[MAX_PLAYERS + 1]

// Needs -> hud + menu + monitor + messages
new nd_menu[MAX_PLAYERS + 1]
new nd_hud_sync
new nd_ent_monitor
new nd_msg_saytext
new nd_msg_damage
...
public zp_extra_item_selected(id, itemid)
{
	if(itemid == g_itemindex1) {
		...
		client_print_color(id, print_team_default, "^4[Fatall-Error] ^1You successfully upgraded your suit!");
...
{
	if (cl_is_thrown[id] != 0 && !zp_get_user_zombie(id))
